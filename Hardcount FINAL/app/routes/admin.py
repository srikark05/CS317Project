...
@admin_bp.route('/admin/players-by-team')
@admin_required
def players_by_team():
    ...
    season_filter = ""
    if season:
        season_filter = "AND pf.season = %s"
        params.append(season)
    rows = run_all(f"""
        SELECT DISTINCT p.name, p.number, p.position
        FROM playsfor pf
        JOIN player p ON p.name = pf.player_name AND p.number = pf.player_number
        WHERE pf.team_id = %s {season_filter}
        ORDER BY p.name
    """, params=params)
    return jsonify([dict(r) for r in (rows or [])])


@admin_bp.route('/admin/add-trade', methods=['GET', 'POST'])
@admin_required
def add_trade():
    teams = run_all("""
        SELECT DISTINCT ON (name) team_id, name FROM team ORDER BY name, team_id
    """)

    if request.method == 'POST':
        try:
            team_from_id   = int(request.form['team_from_id'])
            team_to_id     = int(request.form['team_to_id'])
            trade_date_str = request.form['trade_date'].strip()
            season         = int(request.form['season'])
            cash_str       = request.form.get('cash', '').strip()
            cash           = float(cash_str) if cash_str else None

            from_players = request.form.getlist('from_players')
            to_players   = request.form.getlist('to_players')

            if team_from_id == team_to_id:
                flash('Team From and Team To must be different.', 'error')
                return render_template('admin/add_trade.html', teams=teams)

            if not trade_date_str:
                flash('Trade date is required.', 'error')
                return render_template('admin/add_trade.html', teams=teams)

            trade_date = datetime.strptime(trade_date_str, '%Y-%m-%d').date()
            trade_time = datetime.now()

            from_players_str = ', '.join(from_players) if from_players else ''
            to_players_str   = ', '.join(to_players)   if to_players   else ''

            run_all("""
                INSERT INTO trade
                    (team_from, team_to, trade_date, trade_time,
                     team_from_players, team_to_players, team_to_cash)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
            """, params=(team_from_id, team_to_id, trade_date, trade_time,
                         from_players_str, to_players_str, cash))

            # Move from_players to team_to for this season
            for entry in from_players:
                parts = entry.rsplit(' #', 1)
                if len(parts) != 2:
                    continue
                pname, pnum_str = parts[0].strip(), parts[1].strip()
                ...
                """, params=(pname, pnum, team_to_id, season))

            # Move to_players to team_from for this season
            for entry in to_players:
                parts = entry.rsplit(' #', 1)
                if len(parts) != 2:
                    continue
                pname, pnum_str = parts[0].strip(), parts[1].strip()
                ...
                run_all("""
                    ...
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT DO NOTHING
                """, params=(pname, pnum, team_from_id, season))

            flash('Trade recorded successfully!', 'success')
            return redirect(url_for('admin.dashboard'))

        except Exception as e:
            flash(f'Error recording trade: {e}', 'error')

    today = datetime.now().strftime('%Y-%m-%d')
    return render_template('admin/add_trade.html', teams=teams, today=today)